...
class YOLOLoss(nn.Module):
    ...
    def forward(self, l, input, targets=None):
        ...
        # 计算置信度损失，其中obj_mask是y_true == 1, conf是预测置信度的sigmoid,[noobj_mask.bool() | obj_mask]用于忽略有物体且真值是没物体的
    ...
    def get_target(self, l, targets, anchors, in_h, in_w):
        ...
        for b in range(bs):
            ...
            # -------------------------------------------------------#
            #   将真实框转换一个形式
            #   num_true_box, 4 (0,0,w,h) -> target 以特征图的像素数量
            # -------------------------------------------------------#
            gt_box = torch.FloatTensor(torch.cat((torch.zeros((batch_target.size(0), 2)), batch_target[:, 2:4]), 1))
            # -------------------------------------------------------#
            #   将先验框转换一个形式
            #   num_anchor, 4 -> 特征图像素细度 (0,0,w,h)
            # -------------------------------------------------------#
            ...
            #   此处gt_box 和 anchor_shapes均不包含x,y信息
            #   此处只是为了计算先验框和真实框在特征图细度下的iou, num_target*num_anchors
            # -------------------------------------------------------#
            iou = self.calculate_iou(gt_box, anchor_shapes)
            best_ns = torch.argmax(iou, dim=-1) #最大IOU
            sort_ns = torch.argsort(iou, dim=-1, descending=True) #每个真实框和anchors IOU的排序
            ...
            for t, best_n in enumerate(best_ns):
                ...
                # ----------------------------------------#
                #   用于获得xywh的比例
                #   大目标loss权重小，小目标loss权重大, 该格相对面积大小
                # ----------------------------------------#
    ...
    def get_ignore(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w, noobj_mask):
        ...
        bs = len(targets)
        # -----------------------------------------------------#
        #   生成网格，先验框中心，网格左上角 大小均为 anchors数量*特征图大小
        # -----------------------------------------------------#
        ...
        # 生成先验框的宽高大小 均为 anchors数量
        scaled_anchors_l = np.array(scaled_anchors)[self.anchors_mask[l]]
        anchor_w = torch.Tensor(scaled_anchors_l).index_select(1, torch.LongTensor([0])).type_as(x)
        anchor_h = torch.Tensor(scaled_anchors_l).index_select(1, torch.LongTensor([1])).type_as(x)

        # 重复，形成anchors数量*特征图大小
        anchor_w = anchor_w.repeat(bs, 1).repeat(1, 1, in_h * in_w).view(w.shape)
        anchor_h = anchor_h.repeat(bs, 1).repeat(1, 1, in_h * in_w).view(h.shape)
        # -------------------------------------------------------#
        #   计算调整后的先验框中心与宽高, x,y是预测输出的sigmoid，w,h是预测的输出,最终组合在一起，新xy是预测的中心，wh是预测的wh
        # -------------------------------------------------------#
        ...
        for b in range(bs):
            ...
            #   对于每一张图
            #   将预测结果转换一个形式
            #   pred_boxes_for_ignore     摊平二维->一维 num_anchors也摊平, 4
            # -------------------------------------------------------#
